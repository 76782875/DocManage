...
public class TokenConfig {
    ...
    public static String generateToken(String token, int userId) {
        if (Checker.isNotEmpty(token)) {
            DMApplication.tokens.remove(token);
        }
    ...
    public static String generateToken(int userId) {
        ...
        DMApplication.tokens.put(token, userId);
        saveToken();
        return token;
    }

    public static void saveToken() {
        String tokens = Formatter.mapToJson(DMApplication.tokens);
        try {
    ...
    public static void removeTokenByValue(int userId) {
        if (userId > 0) {
            String removeKey = "";
            for (String key : DMApplication.tokens.keySet()) {
                if (DMApplication.tokens.get(key) == userId) {
                    removeKey = key;
                    break;
                }
            }
            if (Checker.isNotEmpty(removeKey)) {
                DMApplication.tokens.remove(removeKey);
