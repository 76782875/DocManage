...
public class FileServiceImpl implements IFileService {
    ...
    public boolean updateFileInfo(long id, User user, String name, String category, String tag, String description) {
        ...
        if (Checker.isNotNull(file) && file.getIsUpdatable() == 1) {
            ...
            boolean canUpdate = (Checker.isNull(authRecord) ? user.getIsUpdatable() == 1 :
                    authRecord.getIsUpdatable() == 1) && Checker.isNotEmpty(name) && Pattern.compile(DMApplication.settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches();
            if (canUpdate) {
    ...
    public String getResource(String visitUrl, HttpServletRequest request) {
        ...
        boolean downloadable = DMApplication.settings.getBooleanUseEval(ConfigConsts
    ...
    public boolean upload(int categoryId, String tag, String description, String prefix, MultipartFile multipartFile,
        ...
        if (user.getIsUploadable() == 1) {
            ...
            //检测标签是否合法
            if (DMApplication.settings.getBooleanUseEval(ConfigConsts.TAG_REQUIRE_OF_SETTING)) {
                String[] tags = Checker.checkNull(tag).split(ValueConsts.SPACE);
                if (tags.length <= DMApplication.settings.getIntegerUseEval(ConfigConsts.TAG_SIZE_OF_SETTING)) {
                    int maxLength = DMApplication.settings.getIntegerUseEval(ConfigConsts.TAG_LENGTH_OF_SETTING);
                    for (String t : tags) {
