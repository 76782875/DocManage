...
import com.gala.docmanage.dao.UserDAO;
...
import com.gala.docmanage.service.IUserService;
...
public class UserServiceImpl implements IUserService {
    ...
    public User login(String loginName, String password, String token, HttpServletResponse response) {
        boolean allowLogin = DMApplication.settings.getBooleanUseEval(ConfigConsts.ALLOW_LOGIN_OF_SETTINGS);
        User user = null;
        if (allowLogin) {
            if (Checker.isNotEmpty(token) && DMApplication.tokens.containsKey(token)) {
                user = userDAO.getUserById(DMApplication.tokens.get(token));
    ...
    public boolean register(String username, String email, String password) {
        boolean allowRegister = DMApplication.settings.getBooleanUseEval(ConfigConsts.ALLOW_REGISTER_OF_SETTINGS);
        if (allowRegister) {
            boolean isValid = Checker.isEmail(email) && checkPassword(password) && Pattern.compile(DMApplication.settings
    ...
    public boolean checkPassword(String password) {
        int min = DMApplication.settings.getIntegerUseEval(ConfigConsts.PASSWORD_MIN_LENGTH_OF_SETTINGS);
        int max = DMApplication.settings.getIntegerUseEval(ConfigConsts.PASSWORD_MAX_LENGTH_OF_SETTINGS);
